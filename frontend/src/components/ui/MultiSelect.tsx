    ...
    return (
        <div ref={multiSelectRef} className='multi-select-box relative'>
                ...
                {selectedList?.length > 0 && (
                    <div className='flex flex-wrap gap-1'>
                        {selectedList?.map((item: IMultiList) => (
                            <div
                                className='group flex items-center justify-center rounded-full border-[0.7px] border-transparent bg-gray-100  pl-2.5 pr-2 text-sm text-gray-800 hover:border-gray-200 dark:bg-gray-800 dark:text-white/90 dark:hover:border-gray-800'
                                key={item.value}
                            >
                                ...
                                <span>
                                    <X
                                        ...
                                        className='ml-2 h-4 w-4 font-semibold text-gray-700 hover:bg-gray-200 hover:text-red-300'
                                    />
                                </span>
                            </div>
                        ))}
            ...
            <div
                ...
                } absolute top-full z-99999 mt-2 max-h-72 w-full space-y-0.5 overflow-hidden overflow-y-auto rounded-lg border border-gray-200 bg-white p-1 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-gray-300 dark:[&::-webkit-scrollbar-thumb]:bg-neutral-500 [&::-webkit-scrollbar-track]:bg-gray-100 dark:[&::-webkit-scrollbar-track]:bg-neutral-700 [&::-webkit-scrollbar]:w-2`}
            >
                {list.map((item: IMultiList) => (
                    <div
                        onClick={() => handleSelect(item.value)}
                        key={item.value}
                        className='selected focus:outline-hidden z-99999 w-full cursor-pointer rounded-lg px-4 py-2 text-sm text-gray-800 hover:bg-gray-100 focus:bg-gray-100'
                    >
